...
from .poly_slices import extract_poly_slice_err
...
def extract_slice_err(cube, path, spacing=1.0, order=3, respect_nan=True,
                  wcs=None):
    """
    Given an array with shape (z, y, x), extract a (z, n) slice from a path
    with ``n`` segments.
    ...
    if path.width is None:
        ...
        polygons = path.sample_polygons(spacing=spacing, wcs=wcs)
        slice = extract_poly_slice_err(cube, polygons)
